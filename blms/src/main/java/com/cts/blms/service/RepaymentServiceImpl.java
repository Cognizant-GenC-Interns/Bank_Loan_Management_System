...
public class RepaymentServiceImpl implements RepaymentService {
	...
	public void makePayment(Long repaymentId, double amountPaid, LocalDate paymentDate) {
		...
		if (loan.getLoanProduct() == null || loan.getLoanProduct().getTenure() == null
				|| loan.getLoanProduct().getTenure() <= 0) {
		...
		if (loan.getBalance() <= 0.001) {
			...
			int remainingTenure = (int) (loan.getLoanProduct().getTenure()
					- getMonthsBetween(loan.getApprovedDate(), nextRepayment.getDueDate()));
	...
	public void createRepayment(Repayment repayment) {
		repayment.setPaymentStatus(PaymentStatus.PENDING);
		repayment.setInterestAmount(calculateTotalInterest(repayment.getLoanApplication())
	...
	private double calculateTotalInterest(LoanApplication loanApplication) {
		if (loanApplication.getLoanProduct() == null || loanApplication.getLoanProduct().getTenure() == null
				|| loanApplication.getLoanProduct().getTenure() <= 0
				|| loanApplication.getLoanProduct().getInterestRate() == null) {
			return 0.0;
		}
