...
public class ReportController {
    ...
    public ResponseEntity<ByteArrayResource> generateRepaymentReport(@PathVariable Long id){
    	...
    	ByteArrayResource resource = new ByteArrayResource(pdfBytes);
    	return ResponseEntity.ok()
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=Repayment_Report.pdf")
                .contentType(MediaType.APPLICATION_PDF)
                .contentLength(pdfBytes.length)
                .body(resource);
    }
    ...
    public ResponseEntity<ByteArrayResource> getAllOutstandingLoans(){
    	...
    	ByteArrayResource resource = new ByteArrayResource(pdfBytes);
    	return ResponseEntity.ok()
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=Outstanding_Loans.pdf")
                .contentType(MediaType.APPLICATION_PDF)
                .contentLength(pdfBytes.length)
                .body(resource);
    }
...
//        byte[] pdfBytes = reportService.generateLoanReport();
//        ByteArrayResource resource = new ByteArrayResource(pdfBytes);
//
//        return ResponseEntity.ok()
//                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=Loan_Report.pdf")
